...
- **Archetypes:** Single-author (63/63 commits on `HEAD` one author, 76 across all refs, verified 2026-05-16 → bus factor 1 by construction; calibrated per handoff-readiness note) + Greenfield-on-legacy (extracted from `claude-code-tool-dev` monorepo 2026-05-11 → extraction-seam focus).
...
- **HL4a — In-memory replay cache.** Memoize the replay result, invalidate on append (safe — single-writer per session). Applies symmetrically to both stores; pure in-module change. *Unblocks:* the dialogue/delegation scaling surface (each dispatch resolves lineage). *Effort:* small–medium · *Leverage:* medium-high.
