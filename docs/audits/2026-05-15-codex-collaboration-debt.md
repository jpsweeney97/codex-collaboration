...
| Dependency | deep | PyPI surface immaculate (3 pkgs, no CVEs, current). All real risk is the out-of-band Codex CLI. |
...
Promotion `rollback_needed→rolled_back` and `terminal_status="unknown"` emit no `append_audit_event`. *Impact:* the forensic surface most likely consulted first (`audit/events.jsonl`) is silent on the highest-risk operation — a rolled-back promotion looks identical to a clean one. *Fix:* add the `append_audit_event` call at the rollback + unknown-terminal sites (call site + `AuditEvent` schema already exist; batch both). *Effort:* small (~1-2h) · *Leverage:* high.
...
Vendored schemas are used only to assert method-name *presence*; no test validates outgoing `turn/start` / `account/read` / `command-respond` *payload shape* against them. A field rename (`approval_policy`→`approvalPolicy`) passes all 1172 tests; the only catch is `test_codex_compat_live.py`, skipped in CI. *Unblocks:* the entire Codex-CLI cluster — DP-1 pin upgrades (ST2), DP-2 CI gap, OP-3 contract drift, and the security-adjacent `sandbox_policy`/`approval_policy` payload risk. *Fix:* add `jsonschema` dev dep; contract tests that build payloads through `runtime.py` paths and `jsonschema.validate()` against the already-vendored schemas. *Effort:* medium · *Leverage:* high (unblocks dependency + operational + closes a security-adjacent gap).
