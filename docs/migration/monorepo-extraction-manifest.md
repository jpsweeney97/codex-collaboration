...
| `.gitignore` | Standalone repo's gitignore. At extraction, handoffs were tracked (not gitignored). Post-extraction (commit `3563bb3`), `docs/handoffs/` is gitignored going forward. In commit `c0d237c`, the 4 migrated active handoffs were explicitly untracked (`git rm --cached`); the 142 archived migration handoffs remain as the sealed tracked corpus. See `AGENTS.md` Handoffs section for the operational policy. |
...
All rows: classification `migrated-historical`. Rationale: codex-collaboration-era session handoff; project record. Old path `docs/handoffs/archive/<name>` (gitignored in monorepo), new path `docs/handoffs/archive/<name>` (tracked in this repo as part of the sealed migration corpus; the active-handoff reclassification in commit `c0d237c` does not affect these — they remain the canonical sealed historical corpus).
...
**Extraction-method sensitivity:** The counts above are from running `rg` inside the monorepo's working tree, which provides a `.git` context — `rg` respects `.gitignore` only inside a git repository. A `git archive <commit> | tar -x` extract has no `.git`, so `rg` skips the gitignore check and counts one additional matching file: `docs/decisions/2026-03-27-advisory-coherence-after-promotion.md` is tracked at both `701952b8` and `25a5afaf` but matches the repo-root pattern `docs/decisions/*` (the negation `!docs/decisions/2026-04-29-codex-collaboration-drift-synthesis-recovery.md` un-ignores only one specific 2026-04-29 file, not the 2026-03-27 one). Archive-method readings: `701952b8` → 145 unscoped / 144 scoped (vs 144 / 143 above); `25a5afaf` → 138 / 137 (vs 137 / 136 above). The 2-file drift between the original 143 reading (commit `e71db48`) and a separate reviewer's 145 reading at the post-`701952b8` state is one observed instance of this same sensitivity — both readings are correct under their respective extraction methods. The standard is zero stale live authority, not a single canonical count.
