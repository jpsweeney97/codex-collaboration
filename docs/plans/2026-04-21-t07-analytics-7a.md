...
**Path convention:** In this extracted standalone repo, run commands from repo root (`/Users/jp/Projects/active/codex-collaboration`) and use the repo-relative paths below. At original authoring time this plan targeted the monorepo root; old package-root prefixes have been removed where they would mislead current execution.
...
All paths relative to the package root (the repo root in the extracted standalone repo; was `packages/plugins/codex-collaboration/` in the monorepo at the time this plan was written) except docs paths which are repo-root relative.
...
```bash
uv run --package codex-collaboration pytest tests/test_outcome_record.py -v -k "workflow" --tb=short
```
...
```bash
uv run --package codex-collaboration pytest tests/ -q --tb=short
```
...
```bash
git add server/models.py tests/test_outcome_record.py
...
```bash
uv run --package codex-collaboration pytest tests/test_outcome_record.py -v -k "consult_request" --tb=short
```
...
```bash
uv run --package codex-collaboration pytest tests/test_control_plane.py -v -k "workflow" --tb=short
```
...
```bash
uv run --package codex-collaboration pytest tests/test_mcp_server.py -v -k "schema_includes_workflow" --tb=short
```
...
```bash
uv run --package codex-collaboration pytest tests/test_mcp_server.py -v -k "dispatch_passes_workflow" --tb=short
```
...
```bash
git add server/models.py server/control_plane.py server/mcp_server.py tests/test_outcome_record.py tests/test_control_plane.py tests/test_mcp_server.py
...
```bash
uv run --package codex-collaboration pytest tests/test_outcome_record.py -v -k "Delegation" --tb=short
```
...
```bash
uv run --package codex-collaboration pytest tests/test_outcome_shape_consistency.py -v --tb=short
```
...
```bash
git add server/models.py tests/test_outcome_record.py tests/test_outcome_shape_consistency.py
...
```bash
uv run --package codex-collaboration pytest tests/test_journal.py -v -k "Delegation" --tb=short
```
...
```bash
uv run --package codex-collaboration pytest tests/test_delegation_controller.py -v -k "TerminalOutcome" --tb=short
```
...
```bash
uv run --package codex-collaboration pytest tests/test_delegation_controller.py -v -k "RecoveryCatchup" --tb=short
```
...
```bash
mkdir -p skills/codex-analytics
```
...
```bash
uv run --package codex-collaboration pytest tests/test_analytics_skill.py -v --tb=short
```
...
```bash
git add skills/codex-analytics/SKILL.md tests/test_analytics_skill.py
