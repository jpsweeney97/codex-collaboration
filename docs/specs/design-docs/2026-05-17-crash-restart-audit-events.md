...
5. Clean startup emits **no** `crash` and **no** `restart` — *including* a same-session restart with persisted `active` handles and no unresolved journal entries: phase-2 `recover_startup` reattaches those handles, but with no reconciled crash incident this pass it emits nothing.
