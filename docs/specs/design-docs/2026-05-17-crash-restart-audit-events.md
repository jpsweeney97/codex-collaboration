...
- **`turn_dispatch`** — the runtime turn runs *between* the `intent` and `dispatched` writes, so `intent` does **not** imply no-dispatch. `_recover_turn_dispatch` never treats `intent` as a no-op — it verifies via `thread/read` for **both** phases. An unresolved `turn_dispatch` entry means the process died mid-turn → `lineage_handle` `crash` (handle is required → real `runtime_id`, no sentinel); `recovery_result` distinguishes confirmed-finalized from quarantined-`unknown`. No `restart` (nothing is resumed).
...
Governing rule: hook `crash` to the recovery method's own reconcile decision, never to `entry.phase` in the abstract. Emitting `crash` for a pure no-op would be a false forensic record asserting an interruption that did not occur.
...
11. `turn_dispatch:intent` is **not** a no-op: with a `thread/read`-confirmed turn it emits a `lineage_handle` `crash` (real handle `runtime_id`) and finalizes; unconfirmed, it emits `lineage_handle` `crash` with `recovery_result="handle_quarantined_unknown"`. Neither emits `restart`.
