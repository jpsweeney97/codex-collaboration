...
- **B4 (high) — The scheduled sentinel decays exactly when it is needed.** GitHub disables `schedule:` workflows after ~60 days without repository activity — i.e. during the quiet periods this gate exists to cover, reproducing the very "tracking decayed when no human re-ran it" failure one layer up. **Resolution (v1):** a separate push/PR **[drift-analyzer liveness check](#drift-analyzer-liveness-check)** (kept out of source-only Tier 1a) that **warns** — job stays GREEN in v1 — when no successful analyzer run is recorded within `N = 3` days, with manual `workflow_dispatch` recovery. The rolling issue may *display* stale state but **cannot be the detector**: a disabled workflow can't update its own issue. → [Drift-analyzer liveness check](#drift-analyzer-liveness-check).
...
**Cadence — daily.** Observed Codex release velocity is ~0.4 stable releases/day (`0.130`→`0.135` in 12 days), with an `alpha` (`0.136.0-alpha.1`) already staged. Weekly would queue 5–14 versions per run; daily keeps the rolling issue close to reality at negligible CI cost. **Daily cadence is confirmed for v1.**
...
- **Schedule durability (B4)** — GitHub auto-disables `schedule:` workflows after ~60 days without repository activity. Mitigated by a separate push/PR **[drift-analyzer liveness check](#drift-analyzer-liveness-check)** (warn-only in v1); the detector cannot live on the schedule it watches.

## Drift-analyzer liveness check

A lightweight check on **push/PR** — separate from source-only [Tier 1a](#tier-1a--repo-consistency) and from the Tier 2 scheduled job — that detects when the scheduled sentinel has gone dormant. (GitHub disables `schedule:` workflows after ~60 days of repo inactivity, exactly the quiet stretch the gate exists to cover; a push/PR trigger fires regardless of schedule state.)

- **Signal**: query the Tier 2 workflow's last successful run via the **GitHub Actions API**; if none within **`N = 3` days** (daily cadence + weekend/skip margin), emit a visible warning to the job summary.
- **v1 is warn-only**: the job stays **GREEN**. No hard-fail threshold in v1 — a dormant sentinel surfaces as a warning, not a red build on an unrelated push. (A hard-fail age is a possible later addition, deferred to avoid extra policy surface before the analyzer exists.)
- **Permissions**: this job needs `actions: read` to read run history — distinct from the scheduled analyzer's `contents: read, issues: write`.
- **Recovery**: a manual `workflow_dispatch` run of the analyzer records a fresh successful run and resets the clock.
- **Why not the rolling issue**: the detector must live on the push/PR trigger; a disabled scheduled workflow cannot update its own issue, so the issue may *display* stale state but cannot detect it.
...
- `permission_deltas: [{ definition, variant_key, property_path, change, old, new, json_pointer }]` — one entry per added / removed / changed permission/sandbox/approval field, identified by **stable schema identity** — `definition` (e.g. `"SandboxPolicy"`), `variant_key` (the `oneOf` variant's `type`-enum const, e.g. `"workspaceWrite"`; its title as fallback), and `property_path` within the variant (e.g. `"readOnlyAccess"`) — **not** array position. `json_pointer` is a convenience locator only: a raw `oneOf[i]` pointer breaks when upstream reorders variants. E.g. `{definition: "SandboxPolicy", variant_key: "workspaceWrite", property_path: "readOnlyAccess", change: "removed", old: {…}, new: null}`. The comparator reports the change and emits **no** `risk_hint` — `classify_drift` assigns the label, preserving the facts-vs-risk separation B1 exists to create.
...
- **ServerRequest additions** (plugin must *receive/handle*): **`unclassified` in v1**, deterministically — a new ServerRequest method is an unknown that schema cannot assess for reachability, so it routes to human review and is never silently passed. Reachability is owned by [T-20260429-02](../../tickets/2026-04-29-codex-collaboration-unsupported-server-request-reachability.md); once it supplies a reachability registry, this rule can refine to `needs-live-review` vs `unclassified`. *(The label enum and this rule intersect `contracts.md`'s `kind: unknown` contract; routing them to that owner doc is a [deferred](#acceptance-blockers-and-phase-0-prerequisites) item.)*
...
4. Scheduled workflow (Node setup, `permissions: { contents: read, issues: write }`, `concurrency:` group, `workflow_dispatch` recovery) + label-bootstrap and cross-epoch-reopen rolling-issue routing + delta-gated durable comments + job summary; **plus the separate push/PR drift-analyzer liveness check (B4 — `actions: read`, warn-only, `N = 3` days)**.
...
*(Resolved since earlier review rounds: **daily cadence** is confirmed for v1; **blocking-escalation** is no longer open — v1 locks GREEN-with-issue unless the analyzer fails (see [Issue lifecycle](#issue-lifecycle)). The substantive items now live under [Acceptance Blockers](#acceptance-blockers-and-phase-0-prerequisites).)*
