...
**Crash precondition (no crash for never-dispatched work):** `crash` is emitted only when residual state implies a dispatch actually occurred — a journal entry whose terminal unresolved `phase == "dispatched"`, an orphaned active job in `running`/`needs_escalation`, or a lineage handle whose thread was dispatched. A terminal unresolved `phase == "intent"` means dispatch never happened (intent is journaled *before* dispatch, see [§Operation Journal](#operation-journal)); it is reconciled silently with **no** `crash` and **no** `restart`, for every operation. Emitting `crash` for an intent-only reconciliation would assert a runtime interruption that provably did not occur.
