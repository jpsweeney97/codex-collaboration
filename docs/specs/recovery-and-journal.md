...
**Crash precondition (recovery-outcome, not phase):** `crash` is emitted only when an operation's *own recovery path* reconciles in-flight/dispatched state (a non-no-op reconciliation). `phase` is not a universal dispatch proof — the "pure no-op" determination is owned by each recovery method. Verified loci: `thread_creation:intent` and `job_creation:intent` are journaled strictly before any side effect → pure no-ops → no `crash`/`restart`. `turn_dispatch` is never a no-op — the runtime turn runs between the `intent` and `dispatched` writes, so recovery verifies via `thread/read` for both phases and an unresolved entry implies the process died mid-turn (`crash`, real handle `runtime_id`; `recovery_result` distinguishes confirmed from quarantined). `approval_resolution`/`promotion` follow their own delegation-recovery reconcile-vs-no-op classification. Hook `crash` to the recovery method's reconcile decision, never to `entry.phase` in the abstract; emitting it for a pure no-op asserts an interruption that did not occur.
