...
- [ ] **Step 1: Add failing tests**
  ...
  - `turn_dispatch:intent` is **not** a no-op: `thread/read`-confirmed → `lineage_handle` `crash` (real handle runtime) + finalize; unconfirmed → `lineage_handle` `crash` with `recovery_result="handle_quarantined_unknown"`; neither emits `restart`
...
- [ ] **Step 2: Implement event helpers**
  ...
  `lineage_handle` (advisory reattach) — `stem = f"lineage_handle:{handle.collaboration_id}"`. On successful reattach co-emit `crash` then `restart` (`runtime_id` = new resumed runtime; `extra["crash_recovery_key"] = f"{stem}:crash"`); `recovery_result="handle_reattached"`. The `crash` `runtime_id` is the **pre-existing handle's** runtime when a handle existed before recovery, or the sentinel `recovery:unknown-runtime` when the handle was **created during recovery** from a runtime-less `thread_creation:dispatched` entry (sentinel case (b)). On failed reattach emit `crash` only with `recovery_result="handle_quarantined_unknown"`. The stem MUST use `collaboration_id`, never `runtime_id`.
