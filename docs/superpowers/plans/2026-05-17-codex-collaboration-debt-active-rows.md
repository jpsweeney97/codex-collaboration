...
- [ ] **Step 1: Add failing tests**
  ...
  - normal shutdown / clean startup emits no false `crash` and no `restart` — including a same-session restart with persisted `active` handles and no unresolved journal entries: phase-2 `recover_startup` reattaches them, but with no reconciled crash incident this pass it emits nothing (session-partitioned lineage survives a clean restart, so phase-2 reattach alone is not a crash signal)
...
- [ ] **Step 2: Implement event helpers**
  ...
  `lineage_handle` (advisory reattach) — `stem = f"lineage_handle:{handle.collaboration_id}"`. **Two reattach loci:** `thread_creation:dispatched` reattaches *inside phase-1* `_recover_thread_creation` (emit there); `turn_dispatch` is reconciled in phase-1 (no reattach) and reattaches *in phase-2* `recover_startup` (emit there — phase-1 must stage the reconciled incident so phase-2 knows this handle had a crash, since the journal entry may be `completed` by then). On successful reattach co-emit `crash` then `restart` (`runtime_id` = new resumed runtime; `extra["crash_recovery_key"] = f"{stem}:crash"`); `recovery_result="handle_reattached"`. The `crash` `runtime_id` is the **pre-existing handle's** runtime when a handle existed before recovery, or the sentinel `recovery:unknown-runtime` when the handle was **created during recovery** from a runtime-less `thread_creation:dispatched` entry (sentinel case (b)). On failed reattach emit `crash` only with `recovery_result="handle_quarantined_unknown"`. **Gate:** emit only for a handle with a reconciled crash incident this pass — a phase-2 reattach of a clean-session persisted `active` handle (no unresolved entry, no phase-1 quarantine) emits **nothing**. The stem MUST use `collaboration_id`, never `runtime_id`.
